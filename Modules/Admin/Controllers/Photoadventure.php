...
use Config\Services;
use Modules\Admin\Models\PhotoadventureModel;

class Photoadventure extends BackendController {
    ...
    public function __construct(){
        ...
        $this->my_model = new PhotoadventureModel();
    ...
    public function index() {
        ...
        $data['meta_title']		= 'Photo Adventure';
        $data['meta_desc']		= 'Photo Adventure';
        $data['meta_keyword']	= 'Photo Adventure';        
        ...
        if($action_type !=NULL && $arr_ids !=NULL && is_array($arr_ids) && count($arr_ids)>0){
            ...
            return redirect()->to(site_url('admin/photoadventure')); 
        ...
        if($this->request->getVar('keyword')!=NULL){
            $like_cond['tbl_photo_adventure.name'] =  $keyword = $this->request->getVar('keyword');           
        }
        $cond['status !='] = 2;
        ...
        $data['page_heading']	    = 'Photo Adventures';        
    ...
    public function edit(){
        ...
        $data['curr_page'] = ($this->request->getVar('page')!=NULL)?$this->request->getVar('page'):'';
         ...
         if ($this->request->is('post')) {
            ...
            $rules = [
                ...
                "name" => [
                    "label" => "name",                     
                    "rules" => "required|min_length[3]|max_length[200]|is_unique[tbl_photo_adventure.name,tbl_photo_adventure.id,{id} ]"
                ],
                "photo" => [
                    "label" => "Photo", 
           ...
           if ($this->validate($rules)) {
                ...
                $postdata = [
                    ...
                    "is_home" => $this->request->getPost("is_home"),
                ...
                if ($img->isValid() && ! $img->hasMoved()) {
                    ...
                    $img->move(FCPATH . 'uploads/photoadventure/', $newName);

                    $postdata['photo'] = $newName;

                    /** Unlink old image */
                    $old_img = $row->photo;
                    $old_img_path = FCPATH . 'uploads/photoadventure/'.$old_img;
                ...
                $redirect_url = site_url('admin/photoadventure');
        ...
        $is_home = ($this->request->getPost("is_home")!=NULL)?$this->request->getPost("is_home"):''; 
        $data['is_home'] = $is_home;
        $is_menu = ($this->request->getPost("is_menu")!=NULL)?$this->request->getPost("is_menu"):''; 
        $data['is_menu'] = $is_menu; 

        $data['meta_title']		= 'Edit Photo Adventure';
        $data['meta_desc']		= 'Edit Photo Adventure';
        $data['meta_keyword']	= 'Edit Photo Adventure';

        $data['page_heading']	    = 'Edit Photo Adventure';        
    ...
    public function add(){
         ...
         if ($this->request->is('post')) {
            ...
            $rules = [
               "name" => [
                    "label" => "name", 
                    "rules" => "required|min_length[3]|max_length[200]|is_unique[tbl_photo_adventure.name]"
                ],
                "content" => [
                    "label" => "Content", 
                    "rules" => "required"
        ...
        $data['meta_title']		= 'Add Photo Adventure';
        $data['meta_desc']		= 'Add Photo Adventure';
        $data['meta_keyword']	= 'Add Photo Adventure';

        $data['page_heading']	    = 'Add Photo Adventure';        
