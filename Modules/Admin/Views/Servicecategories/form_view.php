...
<?= form_open($row ? 'admin/servicecategories/edit/'.$row['id'] : 'admin/servicecategories/create') ?>
<?= csrf_field() ?>
<?php $r = $row ?? []; ?>
...
<a href="<?= site_url('admin/servicecategories') ?>" class="btn btn-secondary">Cancel</a>
<?= form_close() ?>
</div></div></div>
