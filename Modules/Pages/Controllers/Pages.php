...
class Pages extends FrontendController {
    ...
    public function about(){
        ...
        $sectionModel = new \App\Models\PageSectionModel();
        $page = $pageModel->findBySlug('about');
        $sections = $page ? $sectionModel->getPageSections((int) $page['id']) : [];

        $mission = [];
        if (!empty($sections['mission']['extra_json'])) {
            $mission = json_decode($sections['mission']['extra_json'], true) ?: [];
        }
        $strengths = [];
        if (!empty($sections['core_strengths']['extra_json'])) {
            $strengths = json_decode($sections['core_strengths']['extra_json'], true) ?: [];
        }

        $data = [
            'meta_title'   => $page['seo_title'] ?? 'About — Vidya Infra Construction',
            'meta_desc'    => $page['seo_description'] ?? '',
            'meta_keyword' => $page['seo_keywords'] ?? '',
            'page'         => $page,
            'who'          => $sections['who_we_are'] ?? [],
            'mission_sec'  => $sections['mission'] ?? [],
            'mission'      => $mission,
            'strengths_sec'=> $sections['core_strengths'] ?? [],
            'strengths'    => $strengths,
            'cta'          => $sections['cta'] ?? [],
            'mylib'        => $this->mylib,
            'include'      => 'Modules\\Pages\\Views\\about_views',
        ];
    ...
    */
    ...
    public function contact_us(){
        ...
        $data = [
            'meta_title'   => $page['seo_title'] ?? 'Contact — Vidya Infra Construction',
            'meta_desc'    => $page['seo_description'] ?? '',
            'meta_keyword' => $page['seo_keywords'] ?? '',
            'page'         => $page,
            ...
            'mylib'        => $this->mylib,
            'include'      => 'Modules\\Pages\\Views\\contact_us_views',
        ];
    ...
    public function submit_contact()
    {
        $rules = [
            ...
            'project_type' => 'required|max_length[100]',
        ...
        (new \App\Models\ContactSubmissionModel())->insert([
        ...
        return redirect()->to(site_url('contact'))->with('success', 'Thank you! Your message has been received.');
    }

    public function certificates()
    {
        $page = (new \App\Models\PageModel())->findBySlug('certificates');
        $certs = [];
        for ($i = 1; $i <= 4; $i++) {
            $path = "certificates/c{$i}.jpg";
            if (is_file(ROOTPATH . 'uploaded_folder/' . $path)) {
                $certs[] = $path;
            }
        }

        $data = [
            'meta_title'   => $page['seo_title'] ?? 'Certificates — Vidya Infra Construction',
            'meta_desc'    => $page['seo_description'] ?? 'Compliance, quality and safety certifications held by Vidya Infra Construction.',
            'meta_keyword' => $page['seo_keywords'] ?? '',
            'page'         => $page,
            'certificates' => $certs,
            'include'      => 'Modules\\Pages\\Views\\certificates_views',
        ];
    ...
    public function sitemap()
    {
        ...
        if ($db->tableExists('services')) {
            foreach ($db->table('services')->where('status', 1)->where('deleted_at', null)->get()->getResult() as $s) {
                $urls[] = site_url('services/' . $s->slug);
            }
        }
        if ($db->tableExists('projects')) {
            foreach ($db->table('projects')->where('status', 1)->where('deleted_at', null)->get()->getResult() as $p) {
                $urls[] = site_url('projects/' . $p->slug);
            }
        }
