...
(function(){
  ...
  // Contact form — handled server-side; keep success flash visible
  ...
  const modalServiceId = document.querySelector('#enquiry-service-id');
  document.querySelectorAll('[data-enquire]').forEach(btn=>{
    btn.addEventListener('click', ()=>{
      if(modalService) modalService.value = btn.dataset.enquire;
      if(modalServiceId) modalServiceId.value = btn.dataset.serviceId || '';
  ...
  // Thank-you popup after enquiry submit
  const thankyou = document.querySelector('#thankyou-modal');
  if(thankyou){
    const closeThankyou = ()=> thankyou.classList.remove('open');
    thankyou.addEventListener('click', (e)=>{
      if(e.target === thankyou || e.target.classList.contains('modal-close') || e.target.hasAttribute('data-close-thankyou')){
        closeThankyou();
      }
    });
